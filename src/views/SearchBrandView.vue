  ...
  <div class="row mx-0 mb-0 px-0 bg-dark h-100">
    ...
    <div class="col-8 ">
      <div class="d-flex flex-wrap gap-4 p-3 ms-3">
        <div v-for="brand in brands" :key="brand.accountId" class="d-flex flex-column align-items-center bg-dark p-0 "
          style="width: 200px;">
          <RouterLink :to="{
...
onMounted(async () => {
  const brand_category_result = (await baseApi.get('/brand_category')).data
  const fisrtCategory = brand_category_result.data[0]
  const brand_list_result = (await baseApi.post('/brand_list', {
    category: fisrtCategory
  })).data

  categoryData.value = brand_category_result.data
  brands.value = brand_list_result.data
